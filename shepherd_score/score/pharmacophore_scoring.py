...
"""
from typing import Union, Callable, Literal, Tuple, Optional
...
from shepherd_score.score.constants import P_TYPES, P_ALPHAS, DIRECTIONAL_PTYPES
...
def _compute_pharm_type_vab_only(
    ptype_str: str,
    ptype_1: torch.Tensor,
    ptype_2: torch.Tensor,
    anchors_1: torch.Tensor,
    ...
    vectors_2: torch.Tensor,
    batched: bool,
    cdist_21: torch.Tensor = None,
    vmm_21: torch.Tensor = None,
    allow_antiparallel: bool = False,
) -> torch.Tensor:
    """Compute only VAB (cross-overlap) for a pharmacophore type, skipping self-overlaps."""
    ptype_str = ptype_str.lower()
    ptype_idx = P_TYPES_LWRCASE.index(ptype_str)
    alpha = P_ALPHAS[ptype_str]
    # For batched path, vmm_21 presence signals directional type.
    # For non-batched path, use type name since vmm_21 is always None there.
    has_vectors = vmm_21 is not None if batched else ptype_str in DIRECTIONAL_PTYPES

    if batched:
        mask_1 = torch.where(ptype_1 == ptype_idx, 1, 0)
        mask_2 = torch.where(ptype_2 == ptype_idx, 1, 0)
        if has_vectors:
            VAB = VAB_2nd_order_cosine_mask_batch(
                cdist_21, vmm_21, alpha, allow_antiparallel, mask_1=mask_1, mask_2=mask_2
            )
        else:
            VAB = VAB_2nd_order_mask_batch(cdist_21, alpha, mask_1=mask_1, mask_2=mask_2)
    else:
        mask_inds_1 = torch.where(ptype_1 == ptype_idx)[0]
        mask_inds_2 = torch.where(ptype_2 == ptype_idx)[0]
        if len(mask_inds_1) == 0 or len(mask_inds_2) == 0:
            VAB = 0.
        elif has_vectors:
            VAB = VAB_2nd_order_cosine(
                anchors_1[mask_inds_1], anchors_2[mask_inds_2],
                vectors_1[mask_inds_1], vectors_2[mask_inds_2],
                alpha, allow_antiparallel,
            )
        else:
            VAB = VAB_2nd_order(anchors_1[mask_inds_1], anchors_2[mask_inds_2], alpha)
    return VAB
...
def get_overlap_pharm(ptype_1: torch.Tensor,
    ...
    precomputed_self_overlaps : Optional[Tuple[torch.Tensor, torch.Tensor]], optional
        Pre-computed (total_VAA, total_VBB) self-overlaps. When provided (and
    ...
    _skip_self = precomputed_self_overlaps is not None and not extended_points
    ...
    if batched:
        ...
        if not _skip_self:
            ...
            cdist_22 = cdist_11 = vmm_22 = vmm_11 = None
    else:
    ...
    if 'hydrophobe' in ptype_key2ind:
        if _skip_self:
            VAB = _compute_pharm_type_vab_only('hydrophobe', ptype_1, ptype_2, anchors_1, anchors_2,
                                               vectors_1, vectors_2, batched,
    ...
    if 'znbinder' in ptype_key2ind:
        if _skip_self:
            VAB = _compute_pharm_type_vab_only('znbinder', ptype_1, ptype_2, anchors_1, anchors_2,
                                               vectors_1, vectors_2, batched,
                                               cdist_21=cdist_21 if batched else None)
        elif batched:
    ...
    if 'anion' in ptype_key2ind:
        if _skip_self:
            VAB = _compute_pharm_type_vab_only('anion', ptype_1, ptype_2, anchors_1, anchors_2,
                                               vectors_1, vectors_2, batched,
                                               cdist_21=cdist_21 if batched else None)
        elif batched:
    ...
    if 'cation' in ptype_key2ind:
        if _skip_self:
            VAB = _compute_pharm_type_vab_only('cation', ptype_1, ptype_2, anchors_1, anchors_2,
                                               vectors_1, vectors_2, batched,
                                               cdist_21=cdist_21 if batched else None)
        elif batched:
    ...
    if 'aromatic' in ptype_key2ind:
        if _skip_self:
            VAB = _compute_pharm_type_vab_only('aromatic', ptype_1, ptype_2, anchors_1, anchors_2,
                                               vectors_1, vectors_2, batched,
                                               cdist_21=cdist_21 if batched else None,
    ...
    if 'acceptor' in ptype_key2ind:
        if _skip_self:
            VAB = _compute_pharm_type_vab_only('acceptor', ptype_1, ptype_2, anchors_1, anchors_2,
                                               vectors_1, vectors_2, batched,
                                               cdist_21=cdist_21 if batched else None,
                                               vmm_21=vmm_21 if batched else None,
    ...
    if 'donor' in ptype_key2ind:
        if _skip_self:
            VAB = _compute_pharm_type_vab_only('donor', ptype_1, ptype_2, anchors_1, anchors_2,
                                               vectors_1, vectors_2, batched,
                                               cdist_21=cdist_21 if batched else None,
                                               vmm_21=vmm_21 if batched else None,
                                               allow_antiparallel=False)
    ...
    if 'halogen' in ptype_key2ind:
        if _skip_self:
            VAB = _compute_pharm_type_vab_only('halogen', ptype_1, ptype_2, anchors_1, anchors_2,
                                               vectors_1, vectors_2, batched,
                                               cdist_21=cdist_21 if batched else None,
                                               vmm_21=vmm_21 if batched else None,
                                               allow_antiparallel=False)
    ...
    if precomputed_self_overlaps is not None and not extended_points:
        scores = similarity_func(VAB=overlap, VAA=precomputed_self_overlaps[0], VBB=precomputed_self_overlaps[1])
    else:
